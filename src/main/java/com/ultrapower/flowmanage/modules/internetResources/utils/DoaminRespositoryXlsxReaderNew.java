 ...
 *
 */
public class DoaminRespositoryXlsxReaderNew implements IRowReader {
	private static Logger logger = Logger.getLogger(DoaminRespositoryXlsxReaderNew.class);
	...
	public DoaminRespositoryXlsxReaderNew(SqlSession session, List<FmIpRange> ipRangeList) {
	...
	public void getRows(int sheetIndex, int curRow, List<String> rowlist) {
		try
		{
			...
			//检查第一行的列名及列数是否正确
			if(curRow ==0 && colCount != DOMAIN_RESP_COL_NUM && !checkColume(rowlist))
			{
				throw new Exception("文件格式错误!");
			}else if(curRow > 0 && colCount == DOMAIN_RESP_COL_NUM)
			{
    ...
    */
    ...
    private boolean checkColume(List<String> rowlist)
    {
		...
		if("网站".equals(websiteName) 
				&& "域名".equals(domainName) 
				&& "IP".equals(ip) 
				&& "归属地".equals(attribution))
		{
			flag = true;
		}
